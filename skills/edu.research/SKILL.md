...
>
> **防回滾警語**：**任何**提到 plugin 內部檔案的地方都要用 `${CLAUDE_PLUGIN_ROOT}` 前綴，**包括 prompt 範本內** copy 給 subagent 的字串。修改本檔時請 grep 一下 `references/AGENTS.md` 與 `agents/edu-researcher.md` 字串確認沒有 bare 路徑（v3 timestamp 修補時就在 Step 3 派工 prompt 範本內漏掉一處，xreview 才抓到——別重蹈覆轍）。
...
撰寫前請務必閱讀 ${CLAUDE_PLUGIN_ROOT}/agents/edu-researcher.md 的「硬性上限」段——
核心概念數 ∈ [5, 8]，每段字數目標 100-200（硬性容忍 [95, 200]），引用數 ∈ [5, 10]，
每個概念至少 1 個 unique source（[^3][^3] 算 1 個），
