...
slides.md 中透過 `<!-- _class: xxx -->` 切換版型。有 19 種 class 可用，分為「必用」「推薦」「選用」三層：
...
| `statement` | 單句宣言強調（比 key-point 更大膽） | 橘色底白字大字居中（純 markdown） |
| `intro` | 講者/課程介紹頁 | 左圓形大頭照 + 右側 h2/h3/文字 |
| `two-cols-header` | 標題橫跨 + 下方雙欄 | 跟 `cols` 類似但標題在上方跨全寬。`<div class="left">` + `<div class="right">` |
| `image-text` | 搭配 `![bg left/right]` 使用 | 強化圖文並排時的文字排版（字號微調） |
...
4. **版型要匹配內容**：
   ...
   - 有步驟流程 → 用 `process`
   - 有一句核心結論 → 用 `key-point`
   - 有單句宣言要強調 → 用 `statement`（比 key-point 更視覺衝擊）
   - 有多項重點 → 用 `highlight-box`
   - 有數據 → 用 `big-number`
   - 開場自我介紹 → 用 `intro`
   - 最後 Q&A / 感謝 → 用 `end`
   - 圖文並排需要更好排版 → 用 `image-text`
