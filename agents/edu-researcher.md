...
| 項目 | 下限 | 上限 | 超過 / 不足的處理 |
|---|---|---|---|
| 核心概念數量 | 5 | **8** | 超過 8 → 合併最相關的概念，或把細節塞進其他概念的內文；不足 5 → 補強某個現有概念為兩個 |
| 每個核心概念字數（zh-TW 字元數） | 100 | **200** | 超過 200 → 必須裁剪，刪冗詞、合併句子；可把次要細節移到其他概念或省略 |
| 引用數量（footnote 總數） | 5 | **10** | 超過 10 → **必須挑選最權威的 5-10 條保留，刪除多餘的**；同概念有多個來源時保留 1-2 個最佳的 |
| 每個核心概念 footnote 數 | 1 | **3** | 多於 3 個 footnote 在同段中是 noise，挑最佳保留 |
...
1. 數一數核心概念數量 → 在 [5, 8] 嗎？
2. 數一數每個概念的中文字元 → 在 [100, 200] 嗎？（用估的就好，不必逐字數）
